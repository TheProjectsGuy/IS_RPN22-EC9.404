...
try:
    ...
    from src.starter_code import get_product_functions
...
class NonHoloThreePtBernstein:
    ...
    def __init__(self, n_val = 5) -> None:
        ...
        self.t, self.t_f, self.t_o, self.t_w = \
            sp.symbols("t, t_f, t_o, t_w")  # Time, end, start, wpt.
    ...
    def solve_wpt_constr(self, constrs :dict = dict(), 
        ...
        r"""
            ...
            - x_lm_t: Callable[[float], float]      x(t) function
            - y_lm_t: Callable[[float], float]      y(t) function
            - th_lm_t: Callable[[float], float]     theta(t) function
        """
        ...
        # ===== Return values =====
        # Functions of time (returning float)
        x_lm_t = lambda tval: float(x_ft.subs({t: tval}))
        y_lm_t = lambda tval: float(y_ft.subs({t: tval}))
        th_lm_t = lambda tval: float(th_ft.subs({t: tval}))
        # Return everything
        return x_vals, y_vals, th_vals, t_vals, \
            x_lm_t, y_lm_t, th_lm_t
