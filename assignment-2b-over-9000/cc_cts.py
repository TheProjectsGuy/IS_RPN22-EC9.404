...
"""
    ...
    bounds) using collision cones.
    The script assumes the following:
    - The scaling is applied to robot's velocities. The scaling 's'
        is found using collision cone equations (refer PDF submission)
    - There can be multiple collisions, but the settings should allow
        time scaling as a viable solution (obstacle shouldn't stop on
        path)

    Adjust properties in the following sections
    - ==== User configuration area (robot trajectory) ====
    - ==== User configuration area (obstacle) ====
...
detection_bound = 7    # Sensor for collision check (else scale = 1)
num_sim_samples = 300   # Number of time steps (not for saving!)
ks_val_min = 0.15       # Minimum scaling factor
...
# %% Prepare the system - Collision Cone
...
# Modify LHS
ineq_lhs_orig = (x1-x2)**2 + (y1-y2)**2 - R**2 - \
    (((s*dx1-dx2)*(x1-x2) + (s*dy1-dy2)*(y1-y2))**2/ \
        ((s*dx1-dx2)**2 + (s*dy1-dy2)**2))
...
r1 = (-b - (b**2 - 4*a*c)**0.5)/(2*a)   # Root 1
r2 = (-b + (b**2 - 4*a*c)**0.5)/(2*a)   # Root 2
d = b**2 - 4*a*c    # Discriminant, should be > 0
...
while t_rob_local < t_sim_end:
    ...
    # Time scaling IFF there is a collision (in detection)
    if dist_ro < detection_bound:
        # Values which can be substituted
        subs_sh = {
            R: rob_rad + obs_rad,   # Dialated obstacle radius
            # Robot pose
            x1: r_robot[0], y1: r_robot[1],
            # Robot velocity - in local time (keep track!)
